...
using test2.Migrations.SeedData;
...
namespace test2.Tests
{
    public abstract class test2TestBase : AbpIntegratedTestBase<test2TestModule>
    {
        ...
        protected void UsingDbContext(Action<test2DbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<test2DbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<test2DbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<test2DbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<test2DbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<test2DbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<test2DbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<test2DbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<test2DbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<test2DbContext, Task<T>> func)
        {
            T result;
